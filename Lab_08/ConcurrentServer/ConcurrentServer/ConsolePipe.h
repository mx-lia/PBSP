			...
			while(*((TalkersCmd*)pPrm)!=Exit)
			{
				if(*((TalkersCmd*)pPrm)==Getcommand)
				{
					...
					if (nBytesRead > 0)
					{
						...
						switch(n)
						{
							case 0:	sprintf(WriteBuf, "%s", "Start");
									SetCommand = TalkersCmd::Start;
									break;
							case 1: sprintf(WriteBuf, "%s", "Stop");
									SetCommand = TalkersCmd::Stop;
									break;
							case 2: sprintf(WriteBuf, "%s", "Exit");
									SetCommand = TalkersCmd::Exit;
									break;
							case 3: sprintf(WriteBuf, "\nПодключено: %i\nОшибка: %i\nУспешно: %i\nРаботают: %i\n", Accept, Fail, Finished, Work);
									Check = false;
									break;
							case 4: sprintf(WriteBuf, "%s", "Wait");
									SetCommand = TalkersCmd::Wait;
									break;
							case 5:	sprintf(WriteBuf, "%s", "Shutdown");
									SetCommand = TalkersCmd::Shutdown;
									break;
							default: sprintf(WriteBuf, "%s", "NoCmd");
									Check = false;
									break;
						}
